...
with seeded_cards as (
  ...
  union all select 'foundations-of-transformers', 'Attention as the lesson engine', 'Self-attention basics', 'concept', 1, '{"subtopic_title":"Context weighting","sections":[{"label":"Short explanation","body":"Each token computes how much to focus on all other tokens in the sequence."},{"label":"Formula","body":"Attention(Q, K, V) = softmax(QK^T / sqrt(d_k))V."},{"label":"Intuition","body":"Queries ask what a token wants, keys advertise what tokens contain, and values provide the retrievable information."}]}'::jsonb
  ...
  union all select 'mathematical-foundations-of-transformers', 'Vectors, matrices, and similarity', 'Dot products measure alignment', 'math', 2, '{"subtopic_title":"Similarity scoring","sections":[{"label":"Formula","body":"a · b = Σ_i a_i b_i."},{"label":"Short explanation","body":"The dot product grows when two vectors point in similar directions."},{"label":"Connection to the broader problem","body":"Attention uses dot products to score how relevant one token is to another."}]}'::jsonb
  union all select 'mathematical-foundations-of-transformers', 'Vectors, matrices, and similarity', 'Matrices as batched transformations', 'concept', 3, '{"subtopic_title":"Linear maps","sections":[{"label":"Short explanation","body":"Matrices transform vectors by mixing, scaling, or rotating dimensions."},{"label":"Example","body":"Q = XW_Q, K = XW_K, and V = XW_V apply different learned transformations to the same token matrix."},{"label":"Key takeaway","body":"Matrix multiplication lets transformers process whole sequences in parallel."}]}'::jsonb
  union all select 'mathematical-foundations-of-transformers', 'Softmax and probability flow', 'What softmax does', 'math', 1, '{"subtopic_title":"Normalize scores","sections":[{"label":"Formula","body":"softmax(z_i) = exp(z_i) / Σ_j exp(z_j)."},{"label":"Short explanation","body":"Softmax turns raw scores into non-negative weights that sum to one."},{"label":"Example","body":"A higher compatibility score gives a token a larger share of the attention distribution."}]}'::jsonb
  ...
  union all select 'mathematical-foundations-of-transformers', 'Softmax and probability flow', 'Expectation viewpoint', 'synthesis', 3, '{"subtopic_title":"Weighted averages","sections":[{"label":"Short explanation","body":"Attention outputs are weighted sums of value vectors and can be viewed as expectations over context candidates."},{"label":"Formula","body":"output_i = Σ_j α_ij v_j."},{"label":"Connection to the broader problem","body":"Attention is a differentiable retrieval mechanism built from linear algebra and probability normalization."}]}'::jsonb
  ...
  union all select 'foundations-of-transformer-architecture', 'Multi-head attention and feed-forward blocks', 'Feed-forward layers after attention', 'concept', 2, '{"subtopic_title":"Per-token refinement","sections":[{"label":"Short explanation","body":"A position-wise feed-forward network transforms each token independently after context has been mixed through attention."},{"label":"Formula","body":"FFN(x) = W_2 σ(W_1 x + b_1) + b_2."},{"label":"Intuition","body":"Attention shares information across tokens, then the feed-forward block deepens each token representation."}]}'::jsonb
